...
# FGDS symbols are resolved in load_library_functions(); is_fgds_found()
# reports whether libfgds.so was successfully loaded. This replaces the old
# fgds_ext module probe.
...
def _get_device_id(device: torch_device) -> int:
    """
    get the relative device id from torch device.

    Args:
        device (torch_device): The torch device to get the ID from.

    Returns:
        int: The relative device ID.
    """
    ...
    if idx >= len(entries):
        raise ValueError(
            f"device index {idx} exceeds the range of CUDA_VISIBLE_DEVICES={visible!r}"
        )
    ...
    # UUID ID
    try:
        for phys_idx in range(cuda.device_count()):
            prop = cuda.get_device_properties(phys_idx)
            if getattr(prop, "uuid", None) == target:
                return phys_idx
    except Exception:
        # torch is not available or query failed, fallback to logical index
        pass

    # fallback: cannot parse UUID, return logical index to avoid crashing
    return idx
...
class FgdsFileCopier(CopierInterface):
    ...
    def submit_io(
        ...
        result = _cpp.fgds_regmem(self.device_id, gbuf_ptr, aligned_length, None)
        if result != 0:
