...
// runtime in load_library_functions() alongside cufile; never linked at build
// time. fgds_found mirrors cufile_found for capability probing.
...
static void load_library_functions(const std::string& cudart_override = "") {
    ...
    // FGDS (FGDS_LIB) — Linux-only NVMe-oF GPUDirect path. Resolve its
    // symbols the same way as cuFile/hipFile above. FGDS does not require a
    // GPU runtime to be present, but like GDS it is only meaningful when a
    // GPU is available, so we only attempt the load when gpu_found is set.
    fgds_found = false;
#ifndef _MSC_VER
    if (gpu_found) {
...
// pointers resolved in load_library_functions(). No wrapper object is passed
