...
def _fgds_available() -> bool:
    ...
    try:
        ...
        # FGDS symbols are resolved in load_library_functions(); the classes
        # always exist in the cpp extension, but is_fgds_found() reports
        # whether libfgds.so was actually loaded.
...
def test_new_fgds_copier_fallback_when_unavailable(monkeypatch, framework) -> None:
    ...
    # FGDS symbols are resolved in load_library_functions(); monkeypatch the
    # availability flag and platform so the code reaches the temp-file probe
    # path, where the failing handle is invoked.
